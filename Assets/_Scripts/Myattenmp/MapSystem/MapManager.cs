...
//以后再自定义地图数据
    ...
    public int EliteLayerInterval = 5; // 每5层一个elite

    [Header("地图节点生成设置")]
    public float Normal = 0.5f, Elite = 0.7f, Event = 0.9f;
    ...
    for (int y = 0; y < totalLayers; y++)
    {
        ...
        for (int i = 0; i < nodeCount; i++)
        {
            ...
            else
            {
                ...
                if (rand < Normal) node.roomType = RoomType.Normal;


                else if(rand < Elite)  node.roomType = RoomType.Elite;


                else if(rand < Event)node.roomType = RoomType.Event;

                else node.roomType = RoomType.Normal;
...
private void GenerateConnections()
{
    for (int y = 0; y < totalLayers - 1; y++)//对每个层进行连接生成，最后一层不需要连接
    {
        ...
        foreach (RoomNode currentNode in currentLayer.nodes)//对当前层的每个节点进行连接生成
        {
            ...
            {
                if (Mathf.Abs(nextLayer.nodes[i].x - currentNode.x) <= 5) // x坐标相差不超过3
                {
